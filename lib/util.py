...
def user_dir():
    if "HOME" in os.environ:
        return os.path.join(os.environ["HOME"], ".electrum-BTA")
    elif "APPDATA" in os.environ:
        return os.path.join(os.environ["APPDATA"], "Electrum-BTA")
    elif "LOCALAPPDATA" in os.environ:
        return os.path.join(os.environ["LOCALAPPDATA"], "Electrum-BTA")
    elif 'ANDROID_DATA' in os.environ:
        return "/sdcard/electrum-BTA/"
    else:
...
block_explorer_info = {
    'explorer.batacoin.net': ('http://explorer.batacoin.net',
                        {'tx': 'tx', 'addr': 'address'}),
    'block-explorer.com': ('https://block-explorer.com',
                        {'tx': 'tx', 'addr': 'address'}),
    'Blockr.io': ('https://BTA.blockr.io',
                        {'tx': 'tx/info', 'addr': 'address/info'}),
    'SoChain': ('https://chain.so',
                        {'tx': 'tx/BTA', 'addr': 'address/BTA'}),
}

def block_explorer(config):
    return config.get('block_explorer', 'explorer.batacoin.net')
...
def create_URI(addr, amount, message):
    ...
    p = urlparse.ParseResult(scheme='batacoin', netloc='', path=addr, params='', query='&'.join(query), fragment='')
