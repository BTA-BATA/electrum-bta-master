...
SYSTEM_CONFIG_PATH = "/etc/electrum-BTA.conf"
...
class SimpleConfig(object):
    ...
    def get_wallet_path(self):
        ...
        old_path = os.path.join(self.path, "electrum-BTA.dat")
...
def read_system_config(path=SYSTEM_CONFIG_PATH):
    """Parse and return the system config settings in /etc/electrum-BTA.conf."""
    result = {}
    if os.path.exists(path):
        try:
            import ConfigParser
        except ImportError:
            print "cannot parse electrum-BTA.conf. please install ConfigParser"
            return
...
def read_user_config(path):
    """Parse and store the user config settings in electrum-BTA.conf into user_config[]."""
    if not path:
        return {}
