...
from electrum_bta import SimpleConfig, Wallet, WalletStorage, format_satoshis
from electrum_bta.bitcoin import is_address, COIN
...
def pay_to(recipient, amount, label):
    ...
    droid.dialogCreateSpinnerProgress("Electrum-BTA", "signing transaction...")
    droid.dialogShow()
        ...
        if event["name"] == "click":
            ...
            if id=="buttonPay":
                ...
                if not is_address(recipient):
                    modal_dialog('Error','Invalid BataCoin address')
                    continue
...
def receive_loop():
    ...
    while out is None:
        ...
        if not event:
            ...
            amount = modal_input('Amount', 'Amount you want to receive (in BTA). ', format_satoshis(receive_amount) if receive_amount else None, "numberDecimal")
...
class ElectrumGui:
    ...
    def __init__(self, config, _network):
        ...
        if not storage.file_exists:
            ...
            droid.dialogCreateSpinnerProgress("Electrum-BTA", msg)
            droid.dialogShow()
