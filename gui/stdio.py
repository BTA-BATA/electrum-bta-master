...
_ = lambda x:x
#from i18n import _
from electrum_bta.wallet import WalletStorage, Wallet
from electrum_bta.util import format_satoshis, set_verbosity, StoreDict
from electrum_bta.bitcoin import is_valid, COIN
from electrum_bta.network import filter_protocol
...
class ElectrumGui:
    ...
    def __init__(self, config, network):
        ...
        if not storage.file_exists:
            print "Wallet not found. try 'electrum-bta create'"
            exit()
    ...
    def do_send(self):
        if not is_valid(self.str_recipient):
            print(_('Invalid BataCoin address'))
            return
        try:
    ...
    def network_dialog(self):
        print("use 'electrum-bta setconfig server/proxy' to change your network settings")
        return True


    def settings_dialog(self):
        print("use 'electrum-bta setconfig' to change your settings")
        return True
